...
A case enters a closing period when participant count and summed trust first reach their independent minimums. It stops issuing new assignments, waits until the next shared ten-minute boundary, and then recalculates all votes recorded before that boundary. An exact weighted tie uses the majority among the highest captured level; if that tier is also tied, the case reopens with all ballots preserved. Assignments still open after a final decision keep an outcome-free snapshot. Their later votes cannot change consensus but always receive the current quote and settle immediately against the canonical result. Report-chain settlements are corrected idempotently when an appeal changes that result. All rewards, penalties, stakes, and reversals are written to an append-only Silver ledger with idempotent settlement keys.
