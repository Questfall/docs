...
After both minimums are first met, the case stops issuing new assignments and enters a closing period that ends at the next shared ten-minute market boundary. Every vote recorded before that boundary participates in the final calculation, including votes from assignments beyond the minimum quorum. At the boundary the result is the weighted majority of approve trust versus reject trust. If the two totals are exactly equal, only ballots at the highest captured level are considered. Their simple majority decides the result because all ballots at that level have the same trust weight. If that strongest level is also evenly split, the closing period is cleared, the existing ballots remain, and the case opens again for additional assignments.

A vote recorded exactly at or after the closing boundary is late. It cannot change the case result, but it is still settled against the canonical outcome using the market quote active when the server receives it. Late real votes remain part of the 24-hour capacity measurement and the rolling canonical-outcome balance. For report chains, a later appeal reconciles those settlements idempotently to the newest canonical decision. A moderator who casts a late vote on an earlier stage is dynamically excluded from later appeal assignments.
